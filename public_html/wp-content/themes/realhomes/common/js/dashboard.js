...
(function ($) {
    ...
    var memberships = function () {
        $('#ims-btn-confirm').text(dashboardData.cancel_membership);
    };
    ...
    var stickyHeader = function () {
        var adminbarOffset = $body.is('.admin-bar') ? $('#wpadminbar').height() : 0,
            headerHeight = $siteHeader.outerHeight(),
            headerItem = $siteHeader.find('.rh-header-slim-right'),
            sidebarTopPadding = headerHeight + adminbarOffset;

        $siteHeader.css('top', adminbarOffset).addClass('sticked-header');

        if (767 > $window.width()) {
            var scrolled = false;

            $body.css('padding-top', headerHeight);
            $sidebar.css('padding-top', sidebarTopPadding);

            $(window).on('scroll', function () {
                var scrollTop = $(this).scrollTop();

                if (scrollTop > adminbarOffset) {
                    $siteHeader.css('top', 0);
                    sidebarTopPadding = headerHeight - adminbarOffset;
                } else if (scrollTop < adminbarOffset) {
                    $siteHeader.css('top', adminbarOffset);
                    sidebarTopPadding = headerHeight + adminbarOffset;
                }
                ...
                if (scrollTop >= headerHeight && !scrolled) {
                    ...
                    headerItem.stop().slideUp(200);
                    $sidebar.stop().animate({paddingTop: sidebarTopPadding - headerItem.outerHeight()}, 200);

                } else if (scrollTop < headerHeight) {
                    scrolled = false;

                    headerItem.stop().slideDown(200);
                    $sidebar.stop().animate({paddingTop: sidebarTopPadding}, 200);
                }
            });
        } else {
            if (headerItem.not(':visible')) {
                headerItem.attr('style', '');
            }
            $body.css('padding-top', headerHeight);
            $sidebar.css('padding-top', headerHeight + adminbarOffset);
            $(window).off('scroll');
        }
    };
    ...
    var headerMobileMenu = function () {
        var mobileMenuContainer = $('#rh-main-menus'),
            responsiveMenu = mobileMenuContainer.find('.rh-menu-responsive');
        ...
        mobileMenuContainer.find('#rh-responsive-menu-toggle').on('click', function () {
            $(this).toggleClass('open');
            responsiveMenu.toggleClass('rh-menu-responsive-show');
        });

        $('.rh-menu-responsive > li .rh-menu-indicator').on('click', function () {
            var self = $(this);
    ...
    $document.ready(function () {
        ...
        $propertyActions.find('.delete').on('click', function (event) {
            var link = $(this),
        ...
        $propertyActions.find('.cancel').on('click', function (event) {
            var link = $(this),
                link_parent = link.parents('.property-column-wrap'),
        ...
        $propertyActions.find('.remove-from-favorite').on('click', function (event) {
            var $this = $(this),
                property_item = $this.parents('.property-column-wrap'),
                loader = $this.find('.loader'),
                confirm = $this.find('.confirm-icon'),
        ...
        $document.on('mouseup', function (e) {
            if (!$sidebarDisplayToggle.is(e.target) && $sidebarDisplayToggle.has(e.target).length === 0) {
	...
	// TODO: this migration process is duplicated from favorite properties code in 'common-custom.js' file. Need to make it common.
