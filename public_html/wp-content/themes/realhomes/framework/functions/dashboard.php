...
//		);
		...
		if ( is_user_logged_in() || ( 'yes' !== get_option( 'inspiry_login_on_fav', 'no' ) ) ) {
			$menu['favorites'] = array(
				esc_html__( 'My Favorites', 'framework' ),
				esc_html__( 'My Favorites', 'framework' ),
				'fas fa-heart',
				realhomes_dashboard_module_enabled( 'inspiry_favorites_module_display' )
			);
		}
		...
		if ( is_user_logged_in() ) {
			...
			$menu['profile'] = array(
				...
				esc_html__( 'My Profile', 'framework' ),
				'fas fa-user-alt',
				realhomes_dashboard_module_enabled( 'inspiry_profile_module_display' )
			);
...
if ( ! function_exists( 'realhomes_dashboard_admin_bar' ) ) {
	/**
	 * Hides the admin bar for dashboard template pages.
	 *
	 * @since 3.12
	 *
	 * @param $show
	 ...
	 * @return bool
	 */
	function realhomes_dashboard_admin_bar( $show ) {
	...
	add_filter( 'show_admin_bar', 'realhomes_dashboard_admin_bar' );
}
