...
class Importer {
	...
	public function import_content( $import_file_path ) {
		...
		set_time_limit( apply_filters( 'rhdi/set_time_limit_for_demo_data_import', 300 ) );
		...
		if ( ! apply_filters( 'rhdi/regenerate_thumbnails_in_content_import', true ) ) {
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'rhdi/time_for_one_ajax_call', 25 ) ) {
			$response = array(
				'status'  => 'newAJAX',
			...
			$log_added = Helpers::append_to_file(
				__( 'New AJAX call!' , 'rhdi' ) . PHP_EOL . $message,
