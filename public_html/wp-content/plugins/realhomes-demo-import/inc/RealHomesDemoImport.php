...
class RealHomesDemoImport {
  ...
  private $plugin_page_setup = array();
	 ...
	 *
	 * @return RealHomesDemoImport the *Singleton* instance.
	 */
	...
	public function create_plugin_page() {
		$this->plugin_page_setup = apply_filters( 'rhdi/plugin_page_setup', array(
		...
		$this->plugin_page = add_submenu_page(
			...
			apply_filters( 'rhdi/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) )
		);

		register_importer( $this->plugin_page_setup['menu_slug'], $this->plugin_page_setup['page_title'], $this->plugin_page_setup['menu_title'], apply_filters( 'rhdi/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) ) );
	}
	...
	public function display_plugin_page() {
		require_once RHDI_PATH . 'views/plugin-page.php';
	}
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_enqueue_script( 'ocdi-main-js', RHDI_URL . 'assets/js/main.js' , array( 'jquery', 'jquery-ui-dialog' ), RHDI_VERSIONS );
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'wp_customize_on'  => apply_filters( 'rhdi/enable_wp_customize_save_hooks', false ),
					'import_popup'     => apply_filters( 'rhdi/enable_grid_layout_import_popup_confirmation', true ),
					'theme_screenshot' => $theme->get_screenshot(),
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'rhdi' ),
						'dialog_title'          => esc_html__( 'Are you sure?', 'rhdi' ),
						'dialog_no'             => esc_html__( 'Cancel', 'rhdi' ),
						'dialog_yes'            => esc_html__( 'Yes, import!', 'rhdi' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'rhdi' ),
					),
					'dialog_options' => apply_filters( 'rhdi/confirmation_dialog_options', array() )
				)
			);

			wp_enqueue_style( 'ocdi-main-css', RHDI_URL . 'assets/css/main.css', array() , RHDI_VERSIONS );
		}
	}
	...
	public function import_demo_data_ajax_callback() {
		...
		ini_set( 'memory_limit', apply_filters( 'rhdi/import_memory_limit', '350M' ) );
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'rhdi' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'rhdi' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'rhdi' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'rhdi' ) );
			}
		}
		...
		if ( ! $this->before_import_executed ) {
			...
			/**
			 * 2). Execute the actions hooked to the 'rhdi/before_content_import_execution' action:
			 *
			 * Default actions:
			 * 1 - Before content import WP action (with priority 10).
			 */
			do_action( 'rhdi/before_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
		...
		/**
		 * 4). Execute the actions hooked to the 'rhdi/after_content_import_execution' action:
		 *
		 * Default actions:
		 ...
		 */
		do_action( 'rhdi/after_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		...
		if ( false !== has_action( 'rhdi/after_all_import_execution' ) ) {
	...
	public function import_customizer_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'rhdi/customizer_import_execution', $this->selected_import_files );
		}
	...
	public function after_all_import_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'rhdi/after_all_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. If it did, you can deactivate the %3$sRealHomes Demo Import%4$s plugin, because it has done its job.%5$s', 'rhdi' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'rhdi' ),
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'rhdi', false, plugin_basename( dirname( dirname( __FILE__ ) ) ) . '/languages' );
	}
	...
	public function setup_plugin_with_filter_data() {
		...
		$this->import_files = Helpers::validate_import_file_info( apply_filters( 'rhdi/import_files', array() ) );

		/**
		 * Register all default actions (before content import, widget, customizer import and other actions)
		 * to the 'before_content_import_execution' and the 'rhdi/after_content_import_execution' action hook.
		 */
		$import_actions = new ImportActions();
		$import_actions->register_hooks();

		// Importer options array.
		$importer_options = apply_filters( 'rhdi/importer_options', array(
			'fetch_attachments' => true,
		) );

		// Logger options for the logger used in the importer.
		$logger_options = apply_filters( 'rhdi/logger_options', array(
