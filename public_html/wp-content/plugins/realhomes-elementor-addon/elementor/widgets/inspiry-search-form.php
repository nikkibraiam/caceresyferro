...
class RHEA_Search_Form_Widget extends \Elementor\Widget_Base {
	...
	public function get_inspiry_pages() {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'options'     => $this->get_inspiry_pages(),
			]
		);
		...
		$this->add_control(
			...
			[
				'label'        => esc_html__( 'Display Full With Search Button AT Bottom? ', 'realhomes-elementor-addon' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'label_on'     => esc_html__( 'Yes', 'realhomes-elementor-addon' ),
				'label_off'    => esc_html__( 'No', 'realhomes-elementor-addon' ),
				'return_value' => 'yes',
...
//					'{{WRAPPER}} .rhea_collapsed_search_fields_inner .rhea_prop_search__option' => 'width: {{SIZE}}{{UNIT}};',
	...
	public function rhea_search_location() {
		...
		if ( 'yes' == $get_settings['hide_empty_location'] ) {
			$hide_empty = true;
		} else {
			$hide_empty = false;
		}
...
//		if ( ! is_admin() ) {
		...
		$order = $get_settings['sort_location_alphabetically'];
		if ( 'yes' == $order ) {
		...
		// select boxes names
		$location_select_names = rhea_get_location_select_ids( $this->get_id() );

		// number of select boxes based on theme option
		$location_select_count = rhea_get_locations_number( $get_settings['rhea_select_locations'], 'no' );
...
//		}
	...
	protected function render() {
        ...
        <div class="rhea_search_form_wrapper" id="rhea-<?php echo esc_attr( $this->get_id() ); ?>">
            ...
            <!--            --><?php //var_dump( $this->rhea_search_location()  ); ?>
		...
		if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
            ...
            <script type="application/javascript">
                ...
                rheaLocationField(<?php echo json_encode( $this->rhea_search_location()['all_locations'] )?>,
					<?php echo json_encode( rhea_location_placeholder(
						$settings['rhea_location_ph_1'],
						$settings['rhea_location_ph_2'],
						$settings['rhea_location_ph_3'],
						$settings['rhea_location_ph_4']
					) )?>,
					<?php echo json_encode( $this->rhea_search_location()['select_names'] )?>,
					<?php echo json_encode( $this->rhea_search_location()['locations_in_params'] )?>,
					<?php echo json_encode( $this->rhea_search_location()['select_count'] )?>,
					<?php echo json_encode( $this->rhea_search_location()['any_value'] )?>
                );
            ...
            <script type="application/javascript">
                ...
                jQuery(window).on('load',function () {
                    ...
                    jQuery('.rhea_multi_select_picker_location').selectpicker('refresh');
                });
